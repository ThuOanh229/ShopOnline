...
namespace smart_shop.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Create_EditCategory(loaisp sp)
        {
            ...
            if (sp.MaLoaiSP==0)
            {
                sp.Url = sp.TenLoai.ToFriendlyUrl();
                db.loaisps.Add(sp);
                db.SaveChanges();
            }
            else
        ...
        [HttpPost]
        public ActionResult MemberCreate([Bind(Include = "TenDangNhap,MatKhau,HoTen,NgaySinh,GioiTinh,DiaChi,DienThoai,Email,HinhAnh")] thanhvien thanhvien)
        {
            if (Session["admin"] == null)
                return RedirectToAction("Login");
            if (ModelState.IsValid)
